...
if (isset($_POST['viewid'])) {
    ...
    if (isset($_POST['application_status'])) {
        ...
        // Update the application_status in the registration table
        $query = "UPDATE registration SET application_status = ? WHERE id = ?";
        ...
        if ($result) {
            if ($status == 'Need Correction') {
                // Update document_status to '' in the registration table
                $resubmitQuery = "UPDATE registration SET document_status = '' WHERE id = ?";
                ...
                // Update document_status to 'Pending to Released' in the registration table
                $approveQuery = "UPDATE registration SET document_status = 'Pending to Released' WHERE id = ?";
        ...
        $releaseQuery = "UPDATE registration SET document_status = 'Released', expiration_date = ? WHERE id = ?";
